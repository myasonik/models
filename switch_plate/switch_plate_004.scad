...
//
// v004: PLACEHOLDER 3D emblem raised off the right gang instead of the
// engraving -- a chamfered roundel, pyramidal star, and center dome, to
// exercise a supported face-down print before the real emblem is
// designed. The cover now prints FACE-UP (walls on the bed): supports
// fill the hidden interior cavity and touch only the face's inner
// surface, while the face and emblem print clean on top. Slice the
// cover with supports (interface layer recommended) and a brim for the
// 1.6 mm wall-ring bed contact. The 2D coat-of-arms art is kept below
// for the final emblem. Base still prints support-free, back-down.
//
...
if (part == "cover" || part == "both")
    translate([part == "both" ? 67 : 0, 0, 0])
        cover();   // face-UP: walls on the bed, supports fill the hidden
                   // interior; face and emblem print clean on top
