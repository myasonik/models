...
//
// v023: square again at the new size -- cov_w 125 -> 129. The base
// widens to 125, so the lugs, the props, and the bottom-left relief all
// reach 2 further out on each side. Nothing in the z stack moves.
//
// v024: the real coat of arms replaces the placeholder. A height map of
// the bear, ash tree and fox (frame, field and rock mound cut away)
// comes in through surface() from the coat-of-arms skill, 72 wide on
// the right gang and 4.0 proud at its peak. The placeholder mock and
// the hand-drawn 2D art are gone.
//
// part = "both" lays the two out side by side (~257 mm wide — render one
// part at a time for smaller beds).

part = "both"; // "base" | "cover" | "both"
emblem = true;  // coat-of-arms relief on the right gang; false for a
                // cheaper fit-test print
...
// ---- cover shell ----
cov_w     = 129;     // square with cov_h
cov_h     = 129;     // square; 4 larger than the original 125
...
// ---- coat-of-arms relief (right gang) ----
emb_png  = "../.claude/skills/coat-of-arms/assets/coat_of_arms_relief.png";
emb_px   = [720, 413];  // the map's pixel size; surface() spans px-1
emb_w    = 72;       // image width edge to edge, centered on the gang:
                     // 4.3 clear of the toggle opening, 2.7 short of
                     // the face chamfer
emb_h    = 20*lh;    // 4.0 peak above the face (gray 255). The plaque
                     // carves 7.4 at this width; 54 % keeps it legible
                     // without a finger's width of proudness
emb_sink = 0.05;     // background plane this far inside the face, so
                     // the map's rectangle adds nothing off-figure
emb_s    = emb_w / (emb_px[0] - 1);          // 0.1001 per pixel
...
module cover() {
    union() {
        ...
        // coat-of-arms relief raised off the face (right gang)
        if (emblem) translate([gang_dx, 0, cov_d - emb_sink]) emblem3d();
    }
}

// the relief: an 8-bit height map through surface() -- gray 0..255 is
// z 0..100, the top image row lands at +y, and the solid reaches down to
// z -1 -- scaled to emb_w wide and emb_h proud. The z scale includes
// emb_sink so the peak sits exactly emb_h above the face.
module emblem3d() {
    scale([emb_s, emb_s, (emb_h + emb_sink) / 100])
