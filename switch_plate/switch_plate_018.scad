...
//
// v018: the cover's walls are 2.0 taller and cover the base better.
// Both come from the base: the rim grows 1.6 -> 3.0, which raises the
// face seat, and the back reveal shrinks 0.8 -> 0.2, which drops the
// cover's edge closer to the wall. Wall height 3.2 -> 5.2, cover depth
// 5.2 -> 7.2, exposed base 0.8 -> 0.2. The lug thickness follows the
// rim, so the wedge and the retention lip grow with it.
//
...
base_t    = 12*lh;   // 2.4
rim_t     = 4*lw;    // 1.6
rim_h     = 15*lh;   // 3.0: rim rises base_t..base_t+rim_h = lug height
...
rail_h   = 8*lh;                   // 1.6: half's walls into one piece
                                   // (the props are 5.0 tall now)
rail_x   = 26;
