...
arm_w     = 6;       // arms through the screw pads, side band to spine
pad_d     = 10.5;    // screw pads: cs_d + 1.5 wall each side
    ...
    difference() {
        union() {
            ...
            linear_extrude(base_t) {
                ...
                for (sy = [-1, 1]) {
                    translate([-gang_dx, sy*yoke_dy]) circle(d = pad_d);
...
// spine that props the rib itself (fused 0.1 into the rib bottom)
module print_props() {
    for (y = sac_ys, sx = [-1, 1])
        translate([sx < 0 ? -sac_end : sac_gap, y, 0])
            sac_wall(sac_end - sac_gap);
    rotate([90, 0, 0]) linear_extrude(100, center = true)
        polygon([[-0.3, 0], [0.3, 0], [0.3, rib_bot - 0.5],
                 [0.15, rib_bot + 0.1], [-0.15, rib_bot + 0.1],
                 [-0.3, rib_bot - 0.5]]);
}
