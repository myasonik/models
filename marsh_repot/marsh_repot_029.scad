...
// Spring tongue under each bump [v029]: cantilever cut into the shelf.
tng_len  = 18;     // anchor (front) to free end (slot side)
tng_t    = 1.4;    // flexing thickness left above the under-relief
tng_slit = 1.2;    // slit width around the tongue
tng_free = det_x + det_in + det_top + det_out + 1;   // free-end slit
assert(tng_free - tng_len > -rail_x_end + 2,
       "tongue anchor runs off the front end of the shelf");
assert(tng_t < sill_h - 0.8,
       "tongue too thick: no room for the under-relief");
...
module rails() {
    difference() {
        ...
        // Spring-tongue cuts [v029]: two slits and an under-relief per
        // side free the bump onto a cantilever that deflects instead of
        // jacking the tray.
        tongue_cuts();
    }
}

// Slits and under-relief that turn each bump's shelf patch into a
// tng_t-thick cantilever, anchored at the front, free end past the
// bump toward the slot. All faces vertical in the face print.
module tongue_cuts() {
    x0 = tng_free - tng_len;   // anchor end
    for (m = [0, 1]) mirror([0, m, 0]) {
        // Outer slit: frees the tongue edge from the step base (the
        // shelf's inner edge is already free to the bay).
        translate([x0, rail_lip_in, -1])
            cube([tng_len, tng_slit, sill_h + 1.2]);
        // Free-end slit, transverse.
        translate([tng_free, rail_y_in - 1, -1])
            cube([tng_slit, rail_lip_in + tng_slit - rail_y_in + 1,
                  sill_h + 1.2]);
        // Under-relief: leaves the tongue tng_t thick.
        translate([x0 + 1.5, rail_y_in - 1, -1])
            cube([tng_free - x0 - 1.5 + tng_slit,
                  rail_lip_in + 0.1 - rail_y_in + 1,
                  1 + sill_h - tng_t]);
    }
}
