...
//
// v016: outer surfaces made truly SMOOTH (user request: the chamfered
//   corner bands and coarse rim roundover read as bumps on the edges).
//   - Corners: the v011 chamfer+fillet combo is replaced by single-arc
//     round fillets, r_top 10.4 / r_bot 5 — the original v006 CAD's
//     corner radii. Bottom and top arcs share $fn segmentation, so the
//     hull's corner patches are clean ruled surfaces with no creases.
//   - Rim roundover: swept at 6deg steps (16 slices, was 4).
//   - $fn 96 (was 48).
//   Print cost, accepted deliberately over v011's caution: the two
//   bed-side corner rails get a sub-45deg underside band that widens
//   from ~1.5mm (base end) to ~3mm (rim end) and will sag if printed
//   bare. Those rails are the back face's edges — against the wall in
//   use — and the print already needs interior supports for the
//   far-wall ceiling: extend supports under these two rails, or accept
//   the sag. Interior prisms (floor plinth, slot, tray) keep their
//   chamfered corner language.
...
//   skirt ring continuous. Slide-out is backstopped in use by the wall
//   the pot hangs against (the pull lip stands the pot ~6mm off it);
//   if it will hang free (ceiling hook), add a detent before printing.
...
//   separate pull-out drip tray (basin + face plate + pull lip) under
//   the 5 drainage holes. Visible only from the back.
// v013: flat-rim setback REMOVED. The wall runs uniform 3.2mm thickness
//   to the top, capped by a 3mm outer roundover swept to within 0.2mm
//   of the inner face. Print cost: ~1mm under-curl band on the bed-end
//   rim edge only.
// v012: rim outer edge rounded with rim_r = 1 (superseded by v013).
// v011: shell corners softened with the chamfer+fillet combo [10]
//   (superseded by v016's round corners).
// v010: pot elongated until every wall is at a 5deg draft. Footprints
//   unchanged; height = 11.15/tan(5) = 127.44.
// v009: side-print audit fixes (hex drainage holes, 3.2mm walls, corner
//   chamfers — chamfers reverted in v016). Drain hexes kept.
...
//   Corner radii 10.4/5 — restored in v016.
...
// Pull-out drip tray: basin + face plate + pull lip. Modelled with its
