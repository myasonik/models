...
det_h   = 0.8;     // detent height above the channel floor
    ...
    intersection() {
        union() {
            for (m = [0, 1]) mirror([0, m, 0]) {
                ...
                // Step: lateral guide, filled solid to the skirt wall
                // [v026] so no open trench shows through the bay.
                ...
                // floor, 45deg retaining face toward the slot, gentle
                // ramp toward the front. Spans the tray-bearing zone.
                translate([0, rail_lip_in, sill_h])
                    rotate([90, 0, 0])
                        linear_extrude(height = rail_lip_in - rail_y_in)
                            polygon(det_pts());
            }
        }
