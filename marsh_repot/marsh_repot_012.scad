...
rim_flat  = 1;
setback_h = 4;
rim_r     = 1;   // outer top-edge roundover; consumes the flat and lands
                 // tangent to the setback. Must not exceed rim_flat or
                 // it eats the setback (asserted below).
assert(rim_r <= rim_flat,
       "rim_r > rim_flat would erase the flat rim setback");
...
// inset slices tracing a quarter-round of radius rim_r [v012]. With
// rim_r = rim_flat the roundover lands exactly tangent to the setback.
module outer_shell() {
    hull() {
