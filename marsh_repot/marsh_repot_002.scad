...
// v002: rim cutter reworked — v001 chamfered the outer edge instead of the
// inner one, shaving the top outer rim by 0.56mm per side.
//
...
// Rim-setback cutter: a tapered plug whose walls lean outward at 45 degrees.
// Its bottom sits inside the cavity (cuts nothing); where its slope crosses
// the inner wall it widens the opening toward the rim, leaving a rim_flat
// band on the top face. Extends 1mm above the rim so the cut is clean.
module rim_cutter() {
    ix_top = top_x - 2*rim_flat;
    iy_top = top_y - 2*rim_flat;
    ov = 1;  // overshoot above the rim, slope continued at 45 degrees
