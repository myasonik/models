...
//
// This orientation is load-bearing for the design, not just a
// support-avoidance shortcut: printed upright, the floor plate would have
// to bridge the entire drawer bay cavity below it (open only through the
// 70x9.5mm slot — any support dropped in there would be unreachable to
// remove). Tipping the pot converts both the floor and the slot's roof
// from Z-normal (would-bridge) to near-X-normal (self-supporting) at the
// same time. The cost is that both end walls print with their full
// cross-section stacked flat, layer on layer — v033 addresses that
// directly rather than by hunting for a better rotation angle (checked:
// no single rotation helps the end walls without reintroducing bridging
// risk on the floor/slot, since the two features want opposite rotation
// directions).
//
// v035: finger-pull recess enlarged (user, from a print: opening too
//   narrow to get a finger in; some depth still available; wants a more
//   positive catch). Three params, each checked against the assert it
//   shares before moving:
//   - POCKET_W 40 -> 48: wider mouth. The pot's sill notch is required to
//     stay >= pocket_w+2 (assert), so notch_w follows: 44 -> 52 (still
//     10mm under its own slot_w-8 ceiling). Pocket half-width now 23.2mm
//     inside a 34.5mm-half-width face plate — 11.3mm of material left on
//     each side, unchanged margin logic from before.
//   - POCKET_D 7.2 -> 8.4: deeper. This one had almost no headroom — the
//     web-thickness assert (face_t - face_setback + sill_h*tan(draft) -
//     pocket_d >= 1.6) was passing with only 0.16mm to spare at the old
//     values. face_t grows to match (9.0 -> 10.4) purely to restore
//     margin (0.36mm) for the deeper cut; it doesn't change the plate's
//     visible outer face (that's fixed by the face_x0 draft-plane trim,
//     independent of face_t — face_t only sets how much raw stock exists
//     behind the recess before the trim).
//   - HOOK_D 2.0 -> 3.0: taller lip. pocket_z1 (the hook shelf itself) is
//     already at its own ceiling (face_h - 3, an equality already at
//     v024), so it can't grow without growing face_h, which is set by
//     the pot's slot_open, a bigger cross-part change than asked for.
//     hook_d — the undercut's own depth, i.e. how far the lip actually
//     overhangs for a fingertip to hook under — had plenty of headroom
//     once pocket_d grew (limit is hook_d < pocket_d - 1.6 = 6.8), so
//     that's the lever pulled instead. Verified: 3.0 + 1.6 < 8.4.
// v034: skirt corner softened (user: brim peel chips a corner on the
//   bed-contact wall). Measured the existing G2 blend rather than
//   guessing: at ch_bot=5/blend_l=8 the blend already consumes ~95% of
//   the chamfer edge (only ~0.35mm flat remains total) — the base corner
//   was already close to a true curve, just a SMALL one. Softening it
//   further means growing the radius, not adding more curve to the same
//   size: ch_bot 5 -> 8, blend_l 8 -> 10 together (blend_l has to move
//   with ch_bot or the blend falls behind and leaves MORE flat facet, not
//   less — verified: ch_bot=8 with blend_l left at 8 leaves ~3mm flat).
//   Result: corner reach 7.07 -> 11.31mm, flat facet remaining ~0.97mm
//   total, spreading the curvature (and so the brim-peel stress) over a
//   longer, gentler arc. ch_bot=8 leaves 1.4mm of margin on the
//   slot_w/ch_bot assert below (the hard ceiling is ~9.4mm — the slot and
//   the base corner share the back wall's flat span). blend_l is shared
//   with the rim chamfer (ch_top), so the rim corner also gets a little
//   smoother as a side effect — harmless, not the target of this change.
// v033: end-wall reinforcement (user: back wall on the printer flexes more
//   than the others, and the opposite wall too — both print with their
//   3.2mm cross-section stacked flat, layer on layer, so their strength is
//   pure interlayer bond with no in-plane fiber to help).
//   - END_WALL: back/front wall thickness split out from the shared
//     `wall` (still 3.2, unchanged on the two side walls) into its own
//     end_wall param, 3.2 -> 4.8 (12x a 0.4mm line, same convention as
//     `wall`). More material through the same weak stacking direction
//     still raises the absolute force needed to delaminate it. cavity()
//     now insets asymmetrically (the outline is pre-shrunk in X by
//     2*(end_wall-wall) before the usual `wall`-width offset runs)
//     instead of uniformly; slot_cut's run and tray_len both follow the
//     thicker front/back wall (tray_len 160 -> 157.8, keeping the same
//     ~0.3mm stop clearance the v011 comment established;
//     the old tray_len assert checked against `wall` there, which was
//     wrong once the front wall got its own thickness — fixed to
//     end_wall, same bug class as slot_cut's run).
...
//     unlike the wall itself, Y-normal stays vertical no matter how far
//     the part is tipped for printing (the rotation is about Y only), so
//     each fin prints in the normal ring-stacked orientation and acts as
//     a bonded stringer the wall can't provide for itself. Tapered with
//     the wall's own draft (same hull-of-two-drafted-ends technique as
//     face_x() below) so each fin stays flush against the wall through
//     its full height instead of drifting off it. Kept clear of the
//     drain-hole cutters and the rim roundover in Z (checked by assert),
//     and well inside the side-wall chamfer in Y (checked by assert).
// v032: detent ENLARGED (print feedback: the tray still slides out
//   too easily). det_h 0.6 -> 0.8 (the v027 original; v029 dropped it
//   alongside the spring tongues and v030 kept the lighter bump). The
//   v029 "hard push" complaint was really the ~18mm jacked ride, which
//   v031 fixed — with the ride now a ~4mm blip the taller bump costs
//   little on insertion. det_in stays 0.8, so the retaining face is
//   now a true 45deg (0.6/0.8 was ~37deg); det_out 2.2 -> 3.0 keeps
//   the insertion ramp at ~15deg. Pocket depth and length follow from
//   the parameters.
// v031: v030's pocket-tail extension was WRONG and is reverted (user
//   questioned 4.4 -> 8.9; the question exposed the error). Kinematics:
//   during insertion the bump traverses (in tray coordinates) from the
//   front edge toward the pocket, riding under the SOLID strip between
//   them; the drop is gated by the pocket's FRONT wall, which is the
//   retention face and cannot move without adding seated free play. So
//   the tail extension never changed when the drop happens — the
//   jacked ride equals the bump's distance behind the tray's front
//   edge. Fixed properly: det_x -60 -> -74.5 (bump ~4mm behind the
//   front edge, where the lip is backed by the basin's front wall), so
//   the ride is a ~4mm blip instead of ~18mm; pocket back to its
//   original det_len (+0.5 margin). Retention face, gap, and det_h
//   unchanged.
...
ch_bot  = 8;       // 45deg corner chamfer at the base [v034: 5 -> 8, see
                   // header — grows the corner radius rather than adding
                   // curve to the old, already-near-fully-blended size]
blend_l = 10;      // G2 blend length on each chamfer transition [v024];
                   // 8 -> 10 with ch_bot [v034], so the blend keeps pace
                   // and the base corner stays close to fully rounded
                   // instead of regrowing a flat facet; auto-clamped
                   // where the base chamfer is too short to host it
wall    = 3.2;     // 8x a 0.4mm extrusion line [1]; uniform on the two
                   // SIDE walls (+-Y) incl. corners since v024 (cavity =
                   // parallel inset). See end_wall for the +-X walls.
end_wall = 4.8;    // back/front (+-X) wall thickness [v033]: 12x a 0.4mm
                   // line, same convention as `wall` [1]. These two walls
                   // print with their whole cross-section stacked flat
                   // (see header) so they carry extra material; cavity()
                   // insets X by this instead of `wall`.
...
// --- End-wall gussets [v033] --------------------------------------------
// Vertical fins on the inside of both end walls (floor to near the rim).
// A fin's two large faces are Y-normal, which the print's X-axis
// rotation (see header) never touches — they always print ring-stacked
// and strong, unlike the wall they're bonded to. Purely a stiffener;
// kept out of the drain-hole Z range and the rim roundover.
rib_n    = 5;      // fins per end wall
rib_w    = 2.5;    // fin thickness, Y
rib_d    = 5;      // fin depth, protruding inward from the end wall
rib_z0   = floor_top + 4;    // fin bottom: clear of the drain-hole cutter
rib_z1   = height - 14;      // fin top: clear of the rim roundover
rib_span = 60;                // total fin spread, centered on Y=0
assert(rib_z1 > rib_z0 + 20, "not enough end-wall height for the ribs");
assert(rib_span/2 + rib_w/2 <
       (bot_y + (top_y - bot_y)*rib_z0/height)/2 - wall - 5,
       "end-wall ribs reach the side-wall chamfer");
// --------------------------------------------------------------------------
...
// ch_bot grew in v034 (5 -> 8); this shares the back wall's flat Y-span
// with the slot, so the margin here shrank from 4.4mm to 1.4mm. Ceiling
// is ch_bot ~9.4 before this assert fails outright at the current slot_w.
assert(slot_w/2 <= bot_y/2 - wall - ch_bot,
       "slot_w too wide: slot must stay in the flat of the back wall");
notch_w  = 52;     // sill finger notch below the recess [v028]; 44 -> 52
                   // [v035] to track pocket_w (must stay >= pocket_w+2)
...
det_x   = -74.5;   // steep-face foot, pot X — ~4mm behind the tray's
                   // front edge, so the jacked insertion ride is a
                   // short blip [v031]; the thin lip ahead of the
                   // pocket is backed by the basin's front wall
det_gap = 0.4;     // free play, seated pocket wall to the detent face
det_h   = 0.8;     // detent height above the channel floor [v032]
det_in  = 0.8;     // front-side face run (45deg: the retaining side)
det_top = 0.5;     // plateau
det_out = 3.0;     // slot-side ramp run (~15deg: the insertion side)
det_len = det_gap + det_in + det_top + det_out + 0.5;   // pocket length

// --- Pull-out drip tray (modelled with its bottom at Z0 = as printed;
//     seated in the pot it rides the channel at Z = sill_h) ---
tray_len   = 157.8;  // front edge stops ~0.34mm shy of the far wall when
                     // the face plate is flush with the back at Z0; was
                     // 160/0.26mm before end_wall thickened the front
                     // wall [v033]
...
face_t     = 10.4;   // face plate thickness at its base — thick enough
                     // to hide the finger recess behind a >=1.6mm web;
                     // 9.0 -> 10.4 [v035] to keep that margin once
                     // pocket_d grew
...
// [v023].
pocket_w  = 48;      // opening width, centered; 40 -> 48 [v035]
pocket_d  = 8.4;     // depth, measured from the (drafted) face plane —
                     // a full fingertip pad fits behind the hook [v028];
                     // 7.2 -> 8.4 [v035]
pocket_z1 = 6;       // hook shelf: the bar your fingertip curls behind
hook_d    = 3.0;     // undercut face sits this deep behind the face;
                     // 2.0 -> 3.0 [v035], a taller/more positive catch
...
assert(det_x - det_gap > back_x - tray_len + 0.8,
       "detent too far forward: pocket breaks the tray's front lip");
// The lip ahead of the pocket may be narrow because the basin's front
// wall sits directly above it, stiffening the strip.
...
// the shelf / sill sliding strips at the edges.
...
// Insertion stop: front edge meets the far (thicker) wall's inner face
// at sill_h. Keyed to end_wall, not `wall` — same bug class slot_cut's
// run had before v033: a check sized for the old 3.2mm wall would pass
// even though the front wall is now 4.8mm and the tray's front edge
// would land inside it.
assert(back_x - tray_len >= -(bot_x/2 - end_wall) - sill_h*tan(draft),
...
// Cavity: a true parallel inset (inset = wall) of the outer outlines, so
// the SIDE walls are 3.2mm THROUGH the corners too [v024] — before this
// the top corners ran ~2.3mm and the bottom corners ~4.5mm. The end
// walls (+-X) get extra thickness [v033]: the outline is pre-shrunk in
// X by 2*(end_wall-wall) before the usual `wall`-width offset runs, so
// the offset's proven corner math is untouched and only the flat
// end-wall span (and the diagonal chamfer, which still keys off `wall`)
// picks up the difference.
module cavity() {
    dx = 2*(end_wall - wall);
    hull() {
        translate([0, 0, -eps])
            soft_prism(bot_x - dx, bot_y, eps, ch_bot, blend_l, inset = wall);
        translate([0, 0, height + eps])
            soft_prism(top_x - dx, top_y, eps, ch_top, blend_l, inset = wall);
    }
}
...
// below — nothing overhangs. Unaffected by end_wall [v033]: it hugs the
// OUTER surface at a constant `wall` depth in every direction, so near
// the (now thicker) end walls it simply sits embedded in solid material
// — harmless, since union of solid with solid changes nothing.
module floor_cove() {
    dh = cove_r / cove_n;
    for (i = [0 : cove_n - 1]) {
...
// `run` is sized off end_wall, not `wall` [v033] — the back wall is the
// thicker end wall now, and a cutter sized for the old 3.2mm would stop
// short of actually punching all the way through it.
module slot_cut() {
    run = 10 + end_wall + slot_top*tan(draft) + 5;
...
// End-wall gussets [v033]: a fin per position on each end wall's inside
// face, tapered with the wall's own draft (hull of two correctly-drafted
// end caps, same technique as face_x() below) so it stays flush against
// the wall through its whole height instead of drifting off it as the
// wall leans outward with Z. 1mm of embedding into the wall at each end
// makes the union robust regardless of tessellation.
module rib(y0) {
    x0 = back_x + rib_z0*tan(draft) - end_wall - 1;
    x1 = back_x + rib_z1*tan(draft) - end_wall - 1;
    hull() {
        translate([x0, y0, rib_z0])
            cube([rib_d + 1, rib_w, eps]);
        translate([x1, y0, rib_z1])
            cube([rib_d + 1, rib_w, eps]);
    }
}

module end_wall_ribs() {
    ys = [for (i = [0 : rib_n - 1])
            -rib_span/2 + i*rib_span/(rib_n - 1)];
...
module pot() {
    difference() {
        union() {
            ...
            rails();
            // End-wall gussets: bonded stiffeners on the two weak
            // walls [v033].
            end_wall_ribs();
        }
