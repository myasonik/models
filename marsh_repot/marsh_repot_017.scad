...
//
// v017: protruding pull lip REPLACED by an inset pull (user request:
//   nothing may protrude past the draft plane of the back wall).
//   - The face plate is now a wedge: its outer face lies parallel to
//     the back wall's 5deg draft plane, parked face_setback (0.3mm)
//     shy of it. Since the wall's outermost line is at the rim, the
//     whole tray now sits strictly inside the pot's hanging silhouette.
//   - A finger pocket is recessed into the plate: pocket_w wide,
//     pocket_d deep, with a flat hook shelf at pocket_z1 and a small
//     nib at the shelf's outer edge to catch a fingertip. The plate
//     thickened (face_t 3.2 -> 4.4) to keep a >=1.6mm web behind the
//     pocket. Pocket roof and nib are ~1-2mm bridges in the tray's
//     flat print: clean.
//   - Grip grooves on the tray's underside near the back (the bay is
//     open below): drag the tray out from beneath with a fingertip.
//   Natural stop parks the plate 0.26 deeper than modelled, so the
//   real-world face sits ~0.6mm inside the wall plane. Nothing to
//   catch, nothing proud.
// v016: outer surfaces made truly SMOOTH: single-arc round corner
//   fillets (r_top 10.4 / r_bot 5, the v006 CAD radii), rim roundover
//   swept at 6deg steps, $fn 96. Print cost accepted: the two bed-side
//   corner rails get a sub-45deg underside band (~1.5mm base end to
//   ~3mm rim end) — back edges, hidden in use; support them or accept
//   sag. Interior prisms keep the chamfered corner language.
// v015: the pot HANGS, so the drawer bay carries the tray: two
//   L-section channel rails (shelf reaching rail_under beneath each
//   tray edge + upstand capturing it laterally) run the bay's length,
//   and the slot bottom sits on a solid sill at sill_h, keeping the
//   skirt ring continuous.
...
// v011: chamfer+fillet corner combo [10] (superseded by v016).
// v010: pot elongated until every wall is at a 5deg draft;
//   height = 11.15/tan(5) = 127.44.
// v009: side-print audit fixes (hex drainage holes, 3.2mm walls).
...
//   Skirt is SOLID. Corner radii 10.4/5 — restored in v016.
...
r_top   = 10.4;    // round corner fillet at the rim (v006 CAD value)
r_bot   = 5;       // round corner fillet at the base
...
assert(slot_w/2 <= bot_y/2 - wall - r_bot,
...
face_t     = 4.4;    // face plate thickness at its base — thick enough
                     // to hide the finger pocket behind a >=1.6mm web
face_h     = slot_open - tray_clr;  // 13
face_setback = 0.3;  // face plane parked this far inside the wall plane

// Inset pull: finger pocket recessed into the face plate [v017].
pocket_w  = 40;      // opening width, centered
pocket_d  = 2.2;     // depth, measured from the (drafted) face plane
pocket_z0 = 2.5;     // pocket floor (tray-local Z)
pocket_z1 = 9.0;     // hook shelf: flat roof your fingertip curls behind
nib_d     = 1.0;     // nib at the shelf's outer edge...
...
// Chamfered-rectangle corner points, kept for the interior prisms
// (floor plinth, slot, tray) — the OUTER shell is round-cornered.
...
// Rounded-corner rectangle, extruded: single-arc corner fillets of
// radius r (smooth [v016]). `inset` shrinks the whole outline by a
// parallel offset — used to build the rim roundover slices. Every call
// shares $fn, so stacked/hulled outlines have matching arc vertices and
// the lofted corner patches come out crease-free.
module round_prism(x, y, h, r, inset = 0) {
    linear_extrude(height = h)
        offset(r = r) offset(delta = -r - inset)
...
module outer_shell() {
    hull() {
        round_prism(bot_x, bot_y, eps, r_bot);
        for (t = [0 : 6 : 90])
            translate([0, 0, height - rim_r + rim_r*sin(t) - eps])
                round_prism(top_x, top_y, eps, r_top,
                            inset = rim_r*(1 - cos(t)));
    }
}

// Tapered shell: hull of two round prisms. Matching arc segmentation
// top and bottom keeps the corner patches smooth.
module shell(bx, by, tx, ty, h, rb, rt) {
    hull() {
        round_prism(bx, by, eps, rb);
        translate([0, 0, h - eps]) round_prism(tx, ty, eps, rt);
    }
}
...
// vertical walls, up-facing floors, or 1-2mm bridges in the flat print.
module pocket_cut() {
