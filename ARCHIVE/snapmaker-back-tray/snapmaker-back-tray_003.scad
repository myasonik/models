...
//
// v002: the center screw now seats on a boss at pad-floor level (z=4.2)
// instead of the divider knife-edge. An access bore through the divider
// lets the screw head and driver reach the seat; the head sits flush at
// 4.2, below the bins (they clear it by the same 0.1mm as the pad floors).
//
// v003: center hole re-profiled to the screw spec measured from the
// "U1 Gridfinity Front Tool Mount 6x2" base (same stack as this tray's
// own corner holes): dia8 countersink @45 -> dia3.7 shaft -> 45 reducer
// -> dia2.3 opening, compressed to fit the 4.2mm boss (shaft shortened,
// pilot reduced to the reducer taper; head seating is identical).
//
...
// corner screw holes: countersink dia8 -> shaft dia3.7 -> 45deg -> pilot dia2.3
...
// 4 corner holes, concentric with the socket top corner arcs
...
// center screw, position mapped from rear-tray.3mf (models centered on
// top of each other); hole profile = same screw spec as the corner holes
// (verified against the U1 Front Tool Mount base): dia8 countersink,
// dia3.7 shaft, 45deg reducer toward dia2.3
center_hole  = [0, -9.75];
boss_r       = 7;                    // seat boss around the hole
boss_h       = sock_z - recess;      // 4.2 — flush with the pad floors
bore_r       = 4.6;                  // access bore through the divider
...
// corner countersunk screw hole, top face at z = total_h
module screw_hole() {
...
// center screw cutter: access bore through the divider, then the same
// screw profile as the corner holes seated in the boss top (z = boss_h),
// compressed into the boss height: countersink dia8 -> dia3.7 shaft ->
// 45deg reducer ending at a dia2.3 opening in the underside
module center_screw_hole() {
    cs_depth = cs_r - shaft_r;                  // 2.15
    red_h    = shaft_r - pilot_r;               // 0.7
    translate([0, 0, boss_h - eps])             // access bore to the top
        cylinder(h = total_h - boss_h + 1, r = bore_r);
    translate([0, 0, boss_h - cs_depth])        // dia8 countersink @45
        cylinder(h = cs_depth + eps, r1 = shaft_r, r2 = cs_r);
    translate([0, 0, red_h - eps])              // dia3.7 shaft
        cylinder(h = boss_h - cs_depth - red_h + 2*eps, r = shaft_r);
    translate([0, 0, -eps])                     // 45deg reducer to dia2.3
        cylinder(h = red_h + 2*eps, r1 = pilot_r, r2 = shaft_r + eps);
}
...
difference() {
    union() {
        ...
        // seat boss for the center screw
        translate([center_hole.x, center_hole.y, 0])
            cylinder(h = boss_h, r = boss_r);
    }

    // corner screw holes
    for (p = corner_holes) translate([p.x, p.y, 0]) screw_hole();

    // center screw: bore + countersunk clearance hole
    translate([center_hole.x, center_hole.y, 0]) center_screw_hole();
}

echo(str("Tray: ", outer_len, " x ", outer_wid, " x ", total_h,
         " mm, ", cells, " cells @ ", pitch, "mm"));
echo(str("Center screw at ", center_hole, ": seat z=", boss_h,
         ", countersink dia", 2*cs_r, ", shaft dia", 2*shaft_r,
         ", bottom opening dia", 2*pilot_r, ", bore dia", 2*bore_r));
