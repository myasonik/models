// plant_jigger_007.scad
// v007: de-horn the trough walls. In v006 the crest cut sat at doorway-
//   top height, so wall stock beside the narrowing arch survived as
//   "shoulders" that tapered into thin curved spikes against the dome.
//   Two changes:
//   - the beak's outer stock is capped at the doorway top (archcap2d)
//   - the crest line now starts at crest_root_z (1.7 mm), well below the
//     arch shoulder, so the walls leave the dome on its steep flank and
//     meet it nearly square -- no zero-width tips to print
...
crest_root_z = 1.7;              // wall crest height where the trough
                                 // leaves the dome: kept below the arch
                                 // shoulder so no thin slivers form where
                                 // the walls meet the dome
...
// arch cross-section capped at ztop: the beak's outer stock never rises
// above the doorway top, so the crest cut can't leave thin spikes
module archcap2d(grow, zbot, ztop) {
    intersection() {
        arch2d(grow, zbot);
        translate([-2 * bell_or, zbot]) square([4 * bell_or, ztop - zbot]);
    }
}
...
module spout() {
    difference() {
        // outer beak: capped arch section to spout_mid_y, then taper to
        // the tip. The cap keeps the wall stock at or below the doorway
        // top; the crest cut then defines the final wall-top slope.
        hull() {
            slabY(spout_root_y)
                archcap2d(spout_wall, -skirt_h, arch_cz + arch_r);
            slabY(spout_mid_y)
                archcap2d(spout_wall, -skirt_h, arch_cz + arch_r);
            slabY(spout_tip_y)
        ...
        // crest cut: shave everything above a line falling from
        // crest_root_z at the inner wall to the lip crest -- the trough
        // is open to the sky for its whole length (no roof, no skylight
        // hole), and the walls stay clear of the arch shoulder where
        // they would otherwise thin into spikes against the dome
        hull() {
            slabY(-bell_ir)
                translate([-2 * bell_or, crest_root_z]) square(4 * bell_or);
            slabY(spout_tip_y - 2)
