// Plant Jigger — goblet-shaped double-ended jigger (SPEC.md), v012.
// v011 -> v012: crest plateau removed. In v011 the crest ran flat from the
// wall (y -15..-18) before easing down, so the silhouette read as ledge +
// curve with a visible shoulder. The cosine ease now spans the entire flare
// (emergence tangent point to tip), making the crest one continuous S-curve:
// zero slope where it leaves the bowl, zero slope at the lip, no flat run.
// Everything else (tangent plan hull, minkowski rounding, filleted channel,
// 45-degree lip chamfer) unchanged from v011.
//
...
crest_y0   = -15;            // ease starts at the flare emergence (tangent pt)
...
// crest curve: one cosine ease across the whole flare, emergence to tip
function crest(y) =
