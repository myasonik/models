...
// v003: opacity fix for white-on-black — card back to 5mm thick, plate
//   deepened to 0.9 so every stroke has 1.5mm of white beneath it
//   (white filament lets black bleed through below ~1.2mm). Core
//   between the two pockets stays 2.0mm.
...
card_l   = 65;    // Y
card_t   = 5;     // 5mm buys room for deeper white under the text
corner_r = 3;
margin   = 3;     // side margin for the text
inlay    = 0.6;   // visible letter depth, flush with each face
plate_t  = 0.9;   // buried backing plate; inlay + plate = 1.5mm of white
                  // under every stroke (white goes opaque ~1.2mm; the
                  // 0.6mm of black over the plate is fully opaque)
