...
//
// v015: the pot HANGS, so the tray can no longer rest on a table — the
//   drawer bay now carries it. Two L-section channel rails run the
//   length of the bay: a shelf (Z0..sill_h) reaching rail_under beneath
//   each tray edge, and an upstand beside each edge that captures the
//   tray laterally (rail_gap per side). The slot bottom is raised onto a
//   solid sill at sill_h — the wall below the slot is solid again, so
//   the skirt ring is continuous (v014 had severed it) and the sill top
//   is the channel floor at the back. Rails and sill print as vertical
//   fins in the face print — no new overhangs. Tray heights trimmed to
//   keep the floor lintel: opening 13.5, face 13, basin rim 11. Slide-
//   out is backstopped in use by the wall the pot hangs against (the
//   pull lip stands the pot ~6mm off it); if it will hang free (ceiling
//   hook), add a detent before printing.
...
//   the 5 drainage holes. Visible only from the back. The tray's front
//   edge meeting the far wall is the insertion stop.
// v013: flat-rim setback REMOVED (a deliberate departure from the
//   original Fusion design this file was rebuilt from). The wall runs
//   uniform 3.2mm thickness to the top, capped by a 3mm outer roundover
//   that sweeps to within 0.2mm of the inner face. Print cost: ~1mm
//   under-curl band on the bed-end rim edge only.
...
//   corner_rf = 3; ~0.9mm steep band along the two bed-side corner rails.
...
//   shell corner chamfers). Drain hexes kept.
