...
//
// v020: corner_rf 3 -> 6, killing the "corner nubs". With rf = 3, the
//   two transition fillets and the rim roundover (all r3) met at each
//   rim corner in a marble-sized dome that read as a nub against the
//   big flat faces — the same artifact that was part of the original
//   v013 "bumps" complaint. At rf = 6 the corner turns in one sweep at
//   the scale of the roundover and the nub disappears. Costs, accepted:
//   - The two bed-side corner rails' steep band widens ~0.9 -> ~1.8mm
//     (stick-out ~2.2mm, past the elephant-foot zone): expect a slight
//     under-curl along those two back edges, or paint a support strip
//     on them (the interior ceiling needs supports regardless).
//   - At the base, 2*rf*tan(22.5) = 4.97 ~ ch_bot = 5: the bottom
//     corner chamfer is fully consumed and those corners are simply
//     round (r6). The chamfer strip reads from mid-height up, where
//     ch_top = 10.4 keeps a 5.4mm flat.
...
corner_rf = 6;     // fillet on the chamfer transition edges [10]; big
                   // enough to swallow the rim-corner nub (see v020);
                   // costs ~2mm of under-curl on the bed-side rails
