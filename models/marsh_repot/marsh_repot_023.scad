...
//
// v023: finger pocket FILLETED on every edge (user request). The
//   cutter is rebuilt from three rounded volumes (hulls of r=pocket_fr
//   spheres), so the floor, hook shelf, inner wall, nib faces, and both
//   end walls all carry a pocket_fr fillet, and a fourth rounded volume
//   flares the mouth: its rounded edges intrude pocket_fr past the face
//   plane, easing the entry rim (radius pocket_fr, opening widened by
//   pocket_fr per side at the surface). The nib grew 1.2x0.8 ->
//   1.6x1.6 so a positive fingertip catch survives the rounding; the
//   nib's inner-bottom hook corner sits at the junction of two rounded
//   volumes and stays crisp on purpose. Mouth at the face is now ~41.6
//   x 7.0 (flare included); interior spec dims unchanged. Prints as
//   before: the rounded roof is still a ~3.4mm bridge.
// v022: THE four corner nubs fixed — they were the channel rail
//   shelves, not the rim. Each shelf is a rectangular slab running to
//   x = +-rail_x_end (80) and y = +-rail_y_weld (46.4); at the four
//   plan corners the point (80, 46.4) lies ~1.6mm OUTSIDE the base
//   corner chamfer plane (x + y = 124.2 at Z0), so each shelf corner
//   punched a small tooth through the skirt corner at bed level —
//   present since the rails arrived in v015, visible in the slicer,
//   untouched by every fillet change (v020) and by the rim sweep fix
//   (v021, a real but different artifact). Fixed by intersecting the
//   rails with an inset chamfered prism (footprint - 2.4, same ch_bot
//   chamfer): the weld corners now stop >=1mm behind the outer skin.
//   The clip only trims buried corner material — the tray-bearing
//   shelf surface (|y| <= 34.5) and the upstands are untouched, and
//   the diagonal trim faces print as vertical walls in the face print.
// v021: rim-corner sweep bug fixed — a construction
//   bug, not the fillet size (which is why v020's rf increase didn't
//   help). soft_prism built each roundover slice as
//   offset(r=rf) offset(delta=-rf-inset): shrink first, round after.
//   That keeps the corner arc radius rf CONSTANT at every inset, so at
//   the corners each slice is over-inset by ~0.41*inset (up to 1.2mm at
//   the top slice) relative to a true parallel curve — independent of
//   rf. The hull then bevels the corner cap inward above the crest and
//   the crest corner pokes out: the nub, present since v013. Fixed by
//   parallel-insetting AFTER the rounding: offset(delta=-inset) applied
//   to the finished chamfer+fillet outline. Corner arcs now shrink with
//   inset like a real parallel curve and the roundover is a uniform
//   tube around the whole rim, corners included. corner_rf back to 3
//   (the v011 look; v020's 6 was chasing the wrong cause) — bed-side
//   rail band back to ~0.9mm, base chamfer visible again.
// v020: corner_rf 3 -> 6 attempting to kill the corner nubs (did not
//   work — see v021; superseded).
...
pocket_fr = 0.8;     // fillet on every pocket edge + the entry rim
assert(nib_d + 2*pocket_fr < pocket_d,
       "nib too deep: no room behind it for the filleted hook shelf");
assert(pocket_z1 - nib_h - pocket_z0 >= 2*pocket_fr + 3,
       "mouth too short once filleted: a fingertip no longer fits");
assert(pocket_z0 - pocket_fr >= 1,
       "entry flare cuts into the face plate's bottom rail");
...
// parallel-insets the FINISHED outline — applied after the rounding, so
// every slice is a true parallel curve of the base outline (corner arc
// radii shrink with inset). Insetting before the rounding kept corner
// radii constant and over-inset the corners by ~0.41*inset: that was
// the rim-corner nub, v013-v020 [see v021 note].
module soft_prism(x, y, h, ch, rf, inset = 0) {
    linear_extrude(height = h)
        offset(delta = -inset)
            offset(r = rf) offset(delta = -rf)
...
// Finger pocket cutter, every edge filleted [v023]: three rounded
// volumes (hulls of r=pocket_fr spheres; sphere rows track the drafted
// face plane) replace the sharp extruded J-profile.
//   A1 — mouth-height cavity, floor to the nib's underside, running
//        out past the face;
//   A2 — the tall cavity behind the nib, floor to the hook shelf;
//   B  — entry flare: full-size mouth outline whose rounded inner
//        edges reach pocket_fr past the face plane, easing the rim.
// Their union leaves the nib as a material block whose exposed faces
// are all fillet-bounded; the nib's inner-bottom hook corner (A1-top
// meets A2-wall) stays crisp for grip. In the flat tray print these
// are vertical walls, up-facing floors, and short rounded bridges.
module pocket_cut() {
    fr = pocket_fr;
    yh = pocket_w/2 - fr;
    zm = pocket_z1 - nib_h;   // mouth top = nib underside
    // A1: floor..nib underside, inner wall..out past the face.
    hull()
        for (zc = [pocket_z0 + fr, zm - fr], yc = [-yh, yh],
             dc = [pocket_d - fr, -2])
            translate([face_x(zc) - dc, yc, zc]) sphere(r = fr);
    // A2: floor..hook shelf, inner wall..the nib's inner face.
    hull()
        for (zc = [pocket_z0 + fr, pocket_z1 - fr], yc = [-yh, yh],
             dc = [pocket_d - fr, nib_d + fr])
            translate([face_x(zc) - dc, yc, zc]) sphere(r = fr);
    // B: entry flare across the mouth, rounded rim at the face plane.
    hull()
        for (zc = [pocket_z0, zm], yc = [-pocket_w/2, pocket_w/2],
             dc = [0, -3])
            translate([face_x(zc) - dc, yc, zc]) sphere(r = fr);
}
