...
//
// PRINT ORIENTATION: short side down, rotated 83.93deg about Y so the
// tapered end wall lies flat (90 - 6.07 taper). Hanging tension then
// runs along layer planes [25]. Several v009 choices assume this.
//
...
// Tapered shell with 45deg chamfered corners: hull of two chamfered
// prisms. Chamfers replace v008's corner radii — equal chamfers print
// from any orientation [9], leave no curved rails to layer-step in a
// side print [16], and match the band/tray corner language.
...
module pot() {
    difference() {
        union() {
            ...
            difference() {
                shell(bot_x, bot_y, top_x, top_y, height, ch_bot, ch_top);
                cavity();
            }
            // Floor.
            intersection() {
