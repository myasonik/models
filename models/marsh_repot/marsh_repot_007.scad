...
//
// ONE BODY: the export's two bodies were split only to be instructive;
// the physical part is a single print. The hex band merges into the tray
// plate (band runs to Z=0, buried below the tray floor), and the
// castellation notches rise to Z=3 — 1mm above the tray floor — forming
// water-equalization ports between inside and outside the band (the
// STEP's 192-vert plateau at Z=3 is exactly these notch tops).
//
// v007: merged into one body per user; v006's 0.2mm clearance removed.
...
notch_w   = 1.5;     // castellation port under each hex
notch_top = 3;       // port top: 1mm above the tray floor (STEP Z=3)
...
band_bot = 0;   // band merges into the tray plate
    ...
    difference() {
        union() {
            ...
            difference() {
                ...
                // Castellation ports under each hex: Z 0..notch_top. The
                // lower 2mm is buried in the tray plate; the top 1mm is
                // an open water-equalization port above the tray floor.
                for (i = [0 : n_hex_x - 1])
                    translate([(i - (n_hex_x - 1)/2) * hex_pitch, 0,
                               notch_top/2 - eps])
                        cube([notch_w, band_y + 4, notch_top + 2*eps],
                             center = true);
                for (j = [0 : n_hex_y - 1])
                    translate([0, (j - (n_hex_y - 1)/2) * hex_pitch,
                               notch_top/2 - eps])
                        cube([band_x + 4, notch_w, notch_top + 2*eps],
                             center = true);
            }
        }
