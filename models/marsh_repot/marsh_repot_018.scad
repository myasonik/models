...
ch_top  = 10.4;    // 45deg corner chamfer at the rim [v018: crisp again]
...
// Rectangle with sharp 45deg chamfered corners, extruded. `inset`
// shrinks the whole outline by a parallel offset (edges stay sharp) —
// used to build the rim roundover slices.
module hex_prism(x, y, h, ch, inset = 0) {
    linear_extrude(height = h)
        offset(delta = -inset) polygon(cham_pts(x, y, ch));
}
...
// Bottom and top outlines are both 8-gons with matching vertex order,
// so the hull's wall and chamfer planes are crease-free.
module outer_shell() {
    hull() {
        hex_prism(bot_x, bot_y, eps, ch_bot);
        for (t = [0 : 6 : 90])
            translate([0, 0, height - rim_r + rim_r*sin(t) - eps])
                hex_prism(top_x, top_y, eps, ch_top,
                          inset = rim_r*(1 - cos(t)));
    }
}

// Tapered shell: hull of two chamfered prisms.
module shell(bx, by, tx, ty, h, cb, ct) {
    hull() {
        hex_prism(bx, by, eps, cb);
        translate([0, 0, h - eps]) hex_prism(tx, ty, eps, ct);
    }
}
